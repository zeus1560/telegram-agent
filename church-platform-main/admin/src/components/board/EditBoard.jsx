...
function EditBoard({ open, onClose, menu, boardId }) {
  ...
  useEffect(() => {
    if (boardId !== null) {
      if (open) {
        const load = async () => {
          try {
            const { data } = await boardApi.detail(menu.id, boardId);
            const quill = inputEditor.current.editor;
            quill.clipboard.dangerouslyPasteHTML(0, data.content.content);

            setOrgData(data.content);
            setOrgFiles(data.content.files || []);
            setOrgUploadImages(data.content.images || []);
          } catch (err) {
  ...
  const createFormData = isTemp => {
    ...
    removeFileIds.forEach(fileId => {
      fd.append('removeAttachedFileIds[]', fileId);
    });
  ...
  const saveDraft = async () => {
    try {
      ...
      await boardApi.update(menu.id, boardId, fd);
              ...
              {orgFiles.map((file, index) => (
                ...
                >
                  <AttachedFileButton
                    name={file.originalname}
                    onClickDelete={() => {
                      setRemoveFileIds([...removeFileIds, file.id]);
                      const newList = orgFiles.filter(
                        (_, index2) => index !== index2,
                      );
                      setOrgFiles(newList);
                    }}
