...
const BasicDetail = () => {
  ...
  React.useLayoutEffect(() => {
    const requestList = async () => {
      try {
        const { data } = await boardApi.basicDetail(menuId, id);
  ...
  const moveConent = boardId => {
    navigate(`/menu/${menuId}/board/basic/${boardId}`);
  };
  const handlePrevContent = () => {
    if (prevContent) moveConent(prevContent.id);
  };
  const handleNextContent = () => {
    if (nextContent) moveConent(nextContent.id);
  };
  ...
  const handleClickFile = async file => {
    try {
      const downloadPath = file.location;
      ...
      xhr.onload = () => {
        ...
        a.download = file.originalname || file.location;
        a.click();
      };
      xhr.send();
    } catch (err) {
  ...
  const pdfUrl = content?.files?.find(file => {
    return file.location.indexOf('.pdf') > -1;
  })?.location;
      ...
      {isMobile ? (
        ...
        >
          ...
          >
            ...
            >
              {content?.createdAt
                ? dayjs(new Date(content.createdAt)).format('YYYY.MM.DD')
                : ''}
            </Box>
            <Box
              display="flex"
          ...
          >
            ...
            {pdfUrl ? <PdfRenderer url={pdfUrl} /> : null}
          </Box>
          ...
          <CommentSection menuId={menuId} boardId={id} user={user} />
        ...
        >
          ...
          >
            <Box display="flex">
              ...
              >
                {content?.reg_date && content?.numbering_yn === 'Y'
